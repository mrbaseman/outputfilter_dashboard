...
function opf_select_filters($type='') {
        if($type=='') {
                $res = opf_db_query( "SELECT * FROM `".TABLE_PREFIX."mod_outputfilter_dashboard` ORDER BY `type`,`position` ASC");
        } else {
                $res = opf_db_query( "SELECT * FROM `".TABLE_PREFIX."mod_outputfilter_dashboard` WHERE `type`='%s' ORDER BY `position` ASC", $type);
        }
        if(!$res)
                return(array());
        return($res);
...
function opf_get_data($id) {
        $res = opf_db_query( "SELECT * FROM `".TABLE_PREFIX."mod_outputfilter_dashboard` WHERE `id`=%d", $id);
        if($res) return($res[0]);
        else return($res);
}
...
function opf_get_position($name) {
        ...
        if(opf_is_registered($name, TRUE)) {
                return(opf_db_query_vars( "SELECT `position` FROM `".TABLE_PREFIX."mod_outputfilter_dashboard` WHERE `name`='%s'", $name));
        }
        return(FALSE);
}

// get type
function opf_get_type($name) {
        $name = opf_check_name($name);
        if(!$name) return(FALSE);
        if(opf_is_registered($name, TRUE)) {
                return(opf_db_query_vars( "SELECT `type` FROM `".TABLE_PREFIX."mod_outputfilter_dashboard` WHERE `name`='%s'", $name));
        }
        return(FALSE);
}
...
function opf_is_registered($name, $verbose=FALSE) {
        $name = opf_check_name($name);
        if(!$name) return(FALSE);
        if(opf_db_query_vars( "SELECT TRUE FROM `".TABLE_PREFIX."mod_outputfilter_dashboard` WHERE `name`='%s'", $name))
                return(TRUE);
        else {
                if($verbose)
...
function opf_is_active($name) {
        ...
        if(!$name) return(FALSE);
        if(opf_is_registered($name, TRUE)) {
                if(opf_db_query_vars( "SELECT `active` FROM `".TABLE_PREFIX."mod_outputfilter_dashboard` WHERE `name`='%s'", $name))
                        return(TRUE);
        }
        return(FALSE);
}
...
function opf_set_active($name, $active=1) {
        ...
        if(opf_is_registered($name, TRUE)) {
                return(opf_db_run_query( "UPDATE `".TABLE_PREFIX."mod_outputfilter_dashboard` SET `active`='%s' WHERE `name`='%s'", $active, $name));
        }
        return(FALSE);
}
...
function opf_list_target_modules($sorted=FALSE) { // read from table wb_addons
        $m = array();
        if(!$modules = opf_db_query( "SELECT * FROM  `".TABLE_PREFIX."addons` WHERE `function`='page' ORDER BY `name`"))
                return($m);
        if(!$sorted) {
...
function opf_preload_filter_definitions() {
        ...
        foreach($opf_PAGES as $subpage) {
                ...
                while($trail) {
                        $pid = array_pop($trail);
                        foreach($trail as $p) {
                                //if(!in_array($pid, $opf_PAGECHILDS[$p])) // much more expensive than the array_unique()-calls below!
